...
public class Logger {
  ...
  public final void printLine(String type, String message) {
    printField(type, message + FontInfo.NEWLINE);
  }
  
  public final void printLine(String message) {
    printField(null, message + FontInfo.NEWLINE);
  }
    ...
    if (textPane != null) {
      ...
      if (len > maxBufferSize) {
        try {
          ...
          int offset = text.indexOf(FontInfo.NEWLINE);
          if (offset >= 0) {
            start += offset + 1;
          }
      ...
      if (!current.endsWith(FontInfo.NEWLINE)) {
        current += FontInfo.NEWLINE;
      }
